<style>
    ...
    .sticky-fab {
        ...
        /* Điều chỉnh nếu cần thiết */
        z-index: 1000;
        /* Đảm bảo nó nằm trên nội dung khác */
    }
    ...
    /* CSS cho các nút fab khi hover */
    .fab:hover {
        background-color: #ff6090;
    }

    /* CSS cho các nút fab bên trong fab1 */
    #fab1 .fab {
        position: relative;
        right: 0;
        bottom: 0;
...
<body class="bg-dark-subtle d-flex flex-column min-vh-100">
    <div class="inner-fabs sticky-fab">
        <div class="fab round " id="fab1">
            <i class="material-icons" id="fabIcon">+</i>
        </div>
        <!-- Di chuyển các nút vào trong div inner-fabs -->
        <div id="fab4" class="fab round" data-tooltip="Create"><i class="material-icons">create</i></div>
        <div id="fab3" class="fab round" data-tooltip="Move to inbox"><i class="material-icons">move_to_inbox</i></div>
        <div id="fab2" class="fab round" data-tooltip="Send"><i class="material-icons">send</i></div>
    </div>
                    ...
                    <div class="reading-control text-center" style="font-size: 30px;">
                        <div class="">
                            ...
                            <select id="select-Chapter">
                                ...
                                foreach ($chapters as $key => $value) {
                                ?>
                                    <option value="<?php echo BASE_URL ?>/?url=comicPage/readPage/<?php echo $value['webtoon_id'] . '&chapter=' . $value['id'] ?>">
                                        <?php echo $value['name'] ?>
                                    </option>
                                <?php
                                }
    ...
    <script>
        // JavaScript để ẩn/hiển thị các nút bên trong fab1
        document.getElementById('fab1').addEventListener('click', function() {
            let innerFabs = document.querySelectorAll('.inner-fabs .fab');
            innerFabs.forEach(function(fab) {
                fab.style.display = (fab.style.display === 'none' || fab.style.display === '') ? 'inline-flex' : 'none';
            });
        });
    </script>
